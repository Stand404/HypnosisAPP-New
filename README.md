...
5. **全屏设置限制**: 全屏功能仅在 Android 和 iOS 平台有效，移动端Web 和桌面平台暂时不支持全屏设置(MacOS本身可通过最大化来达到全屏的效果)

6. **桌面端窗口缩放问题**: 在桌面端（Windows、macOS、Linux）使用时，动画会根据窗口大小渲染以节省资源，但不会监听窗口大小变化。如果在小窗口打开应用后放大窗口，可能会出现空白区域。此时需要切换动画类型来触发动画重绘刷新
